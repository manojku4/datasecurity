 ...
 * @author java4
 */
public class Download_ceaser extends HttpServlet {
  ...
  HttpSession session = request.getSession(true);
       String  uname=(String)session.getAttribute("ssname");
        int fileid1 =  Integer.parseInt(request.getParameter("id"));
          
          System.out.println("select * from uploads where fileid="+fileid1+"");
          
          ResultSet rt=st.executeQuery("select * from uploads where fileid='"+fileid1+"'");
        ...
        int shiftKey=1;
         String content = new CaesarCipher().decrypt(sb.toString(), shiftKey);
